...
function generateDailyReportEmail(data) {
  const { username, date, sessions, totalMs, earnings, hourlyRate } = data;
    ...
    <div style="background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
      ...
      <div style="background-color: #FAF8F5; padding: 20px; text-align: center;">
        <p style="margin: 0; color: #999; font-size: 12px;">
          Sent from Clock In Time Tracker
        </p>
      </div>
    </div>
  </div>
...
function generateWeeklyReportEmail(data) {
  const { username, weekStart, weekEnd, sessions, totalMs, earnings, hourlyRate, dailyStats, projectBreakdown } = data;

  const days = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
  const maxDailyMs = Math.max(...dailyStats, 1);

  const dailyBarsHtml = dailyStats.map((ms, i) => {
  ...
  const projectsHtml = projectBreakdown.map(p => `
    <tr>
